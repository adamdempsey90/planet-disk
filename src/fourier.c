...
int main(void) {
	...
	Ny = fld->Ny;
	Nk = fld->Nk;
	double complex *uv = (double complex *)malloc(sizeof(double complex)*Nx*NC);
	...
	for(i=0;i<Nx;i++) {
		for(j=0;j<NC;j++) {
			...
			fprintf(rfp1,"%lg ",creal(fld->v[CINDX]));
			fprintf(cfp1,"%lg ",cimag(fld->v[CINDX]));
		}
		fprintf(rfp1,"\n"); fprintf(cfp1,"\n");
		fprintf(rfp2,"\n"); fprintf(cfp2,"\n");
	}
...
/* Form product in real space */
	for(i=0;i<NTOTR;i++) wr3[i] = wr1[i]*wr2[i];
...
void init_Field(Field *fld) {
	...
	fld->u = (double complex *)malloc(sizeof(double complex)*NC);
	fld->v = (double complex *)malloc(sizeof(double complex)*NC);
	fld->vx = (double *)fld->u;
	fld->vy = (double *)fld->v;
	...
	for(i=0;i<fld->Nx;i++) {
		...
		for(j=0;j<NC;j++) {
			k = fld->k[j];
			ld = M_PI/pow(k*k*.0024,1./3);
			fld->u[CINDX] = csqrt(x)*cexp((2.0/(3.0*k))*I*x*x)*exp(-pow(x/ld,3.));
			fld->v[CINDX] = cexp((2.0/(3.0*k))*I*x*x)*exp(-pow(x/ld,3.))/csqrt(x);	
		}
	}
