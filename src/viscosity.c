...
/* Forms Navier-Stokes stress tensor T=grad(v) + grad(v)^T - 2/3 div(v) I */
	...
	convolve(fld->Tens->Txx,fld->sig,fld->Tens->Pixx,fld->Params->nu);
	convolve(fld->Tens->Txy,fld->sig,fld->Tens->Pixy,fld->Params->nu);
	convolve(fld->Tens->Tyy,fld->sig,fld->Tens->Piyy,fld->Params->nu);

/* Add the Pressure stress and the stress due to the shear 
	Pi = -c^2 d I + nu*d*T */
