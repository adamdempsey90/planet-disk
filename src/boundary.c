...
//  			fld->u[CINDX] = fld->u[indx];
//  			fld->v[CINDX] = fld->v[indx];
//  			fld->sig[CINDX] = fld->sig[indx];
// 
// 		}
// 	}	
...
void wavekillbc(Field *fld,double dt)
{
	int i,j;
	double R,tau,x;
	double x_inf = fld->x[NG] + (fld->Params->Lx)*0.1;
	double x_sup = fld->x[Nx+NG-1] - (fld->Params->Lx)*0.1;
#ifdef OPENMP 
